...
namespace milg::audio {
    ...
    public:
        template<std::size_t output_bus, std::size_t input_bus>
        bool attach_output(std::shared_ptr<Node> input) {
            ...
            return ma_node_attach_output_bus(
                this->get_handle(), output_bus,
                input->get_handle(), input_bus
            ) == MA_SUCCESS;
        }
        ...
        bool detach_output(std::size_t output_bus) {
            ...
            return ma_node_detach_output_bus(
                this->get_handle(), output_bus
            ) == MA_SUCCESS;
        }
    protected:
        template<std::size_t input_bus, std::size_t output_bus>
        bool map_input(std::shared_ptr<Node> output) {
        ...
        template<std::size_t output_bus, std::size_t input_bus>
        bool map_output(std::shared_ptr<Node> input) {
