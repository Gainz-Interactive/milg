...
static void on_process(ma_node      *node,
                       const float **frames_in,
                       ma_uint32    *n_frames_in,
                       float       **frames_out,
                       ma_uint32    *n_frames_out)
{
    ma_vocoder_node *vocoder_node = (ma_vocoder_node *)node;

    voclib_process(&vocoder_node->voclib,
                   frames_in[0], frames_in[1],
                   frames_out[0], *n_frames_out);
}

static ma_node_vtable node_vtable = {
    on_process,
    NULL,
    2,
    1,
...
VocoderNode::VocoderNode()
    : node(std::make_unique<ma_vocoder_node>())
{
    ...
    auto waveform_config = ma_waveform_config_init(ma_format_f32, 1, sample_rate, ma_waveform_type_sawtooth, 2.0, 100);
